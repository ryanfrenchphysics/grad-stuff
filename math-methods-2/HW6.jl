...
Base.init_load_path()
Base.reinit_stdio()	# Included so that IO still initializes when compiled
...
Base.@ccallable function julia_main(ARGS::Vector{String})::Cint
    try
        ...
        Base.inkvokelatest(Base.display_error, Base.catch_stack())
        return 1
    end
    return 0
end
...
function main()
    ...
    if parsed_args["scaled_plots"] != nothing
        SCALEDPLOTS = parsed_args["scaled_plots"]
    end
